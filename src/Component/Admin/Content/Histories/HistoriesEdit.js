...
import {
  saveHistories,
  getHistory
} from "../../../../Services/Histories/histories";
//import { showAlertMSG } from "../../../../Lib/helpers";

function HistoriesEdit(props) {
  ...
  const ref_close_price = useRef(null);
  const ref_close_time = useRef(null);
  ...
  const [close_priceSelected, setclose_priceSelected] = useState(-1);
  const [close_timeSelected, setclose_timeSelected] = useState(-1);
  const [profitSelected, setprofitSelected] = useState(-1);
  const [swapSelected, setswapSelected] = useState(-1);
  // const [sortedcategory, setsortedcategory] = useState([]);

  const getHistoryData = async () => {
    let retData = await getHistory(props.data);
    if (retData !== null) {
      ...
      setclose_priceSelected(retData.row.close_price);
      setclose_timeSelected(retData.row.close_time);
  ...
  const saveData = () => {
    ...
    let data = {
      ...
      close_price: ref_close_price.current.value,
      swap: ref_swap.current.value,
      close_time: ref_close_time.current.value,
  ...
  return (
    <div style={{ backgroundColor: "#fff" }}>
      <form>
        ...
        <div className="form-group">
          <label htmlFor="for_state">Close Price</label>
          <select
            ref={ref_close_price}
            value={close_priceSelected}
            id="for_state"
            className="form-control"
            onChange={(e)=>{
                setclose_priceSelected(e.target.value);
            }}
          >
            <option value="0">Hide</option>
            <option value="1">Show</option>
        ...
        <div className="form-group">
          <label htmlFor="for_state">Close Time</label>
          <select
            ref={ref_close_time}
            value={close_timeSelected}
            id="for_state"
            className="form-control"
            onChange={(e)=>{
                setclose_timeSelected(e.target.value);
            }}
          >
            <option value="0">Hide</option>
            <option value="1">Show</option>
        ...
        <div className="form-group">
          <label htmlFor="for_state">Profit</label>
          <select
            ref={ref_profit}
