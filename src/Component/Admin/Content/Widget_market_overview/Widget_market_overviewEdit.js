...
import {
  saveWidget_market_overviews,
  getWidget_market_overview
} from "../../../../Services/Widget_market_overview/widget_market_overview";
//import { showAlertMSG } from "../../../../Lib/helpers";

function Widget_market_overviewEdit(props) {
  const ref_title = useRef(null);
  const ref_script_url = useRef(null);
  const ref_light_content = useRef(null);
  const ref_dark_content = useRef(null);
  const ref_my_symbol = useRef(null);
  const ref_status = useRef(null);

  const [statusSelected, setstatusSelected] = useState(-1);
  const [my_symbolSelected, setmy_symbolSelected] = useState(-1);
  // const [sortedcategory, setsortedcategory] = useState([]);

  const getWidget_market_overviewData = async () => {
    let retData = await getWidget_market_overview(props.data);
    if (retData !== null) {
      ...
      setmy_symbolSelected(retData.row.my_symbol);
  ...
  useEffect(() => {
    if (props.data !== 0) {
      getWidget_market_overviewData(props.data);
    }
  }, []);
  ...
  const saveData = () => {
    ...
    let data = {
      ...
      my_symbols: ref_my_symbol.current.value
    };
    saveWidget_market_overviews(data);
  };
  return (
    <div style={{ backgroundColor: "#fff" }}>
      <form>
        ...
        <div className="form-group">
          <label htmlFor="for_my_symbol">Show Current Symbol Data</label>
          <select
            ref={ref_my_symbol}
            value={my_symbolSelected}
            id="for_my_symbol"
            className="form-control"
            onChange={(e)=>{
                setmy_symbolSelected(e.target.value);
            }}
          >
...
export default Widget_market_overviewEdit;
